...
import type { KeywordDetailPayload, KeywordSample } from "@/types/keyword-serp";
...
function toSamplePayload(sample: KeywordSerpSample): KeywordSample {
...
export async function GET(
  ...
  if (!rows.length) {
    return NextResponse.json<KeywordDetailPayload>({
      keywordId,
      keywordTerm: null,
      keywordMarket: null,
  ...
  const snapshot = (summaryRow?.snapshot as Record<string, unknown> | undefined) ?? undefined;
  const directTotalResults = typeof summaryRow?.total_results === "number" ? summaryRow.total_results : null;
  const snapshotTotalResults = snapshot && typeof snapshot.totalResults === "number" ? snapshot.totalResults : null;
  const firstSample = limitedSamples[0];
  const sampleTotalResults = firstSample && typeof firstSample.totalResults === "number" ? firstSample.totalResults : null;
  const totalResults = directTotalResults ?? snapshotTotalResults ?? sampleTotalResults ?? null;
