...
async function main() {
  ...
  try {
    ...
    const testText = "Handmade leather wallet for men - premium quality, minimalist design";
    ...
    const { data: queryData, error: queryError } = await supabase
      .from("ai_corpus")
      .select("id, source_type, chunk, metadata, owner_scope, marketplace")
      .eq("id", testId)
      .single();

    if (queryError) {
      throw new Error(`Failed to query test record: ${queryError.message}`);
    }
    ...
    console.log(`  - Chunk: "${queryData.chunk.substring(0, 50)}..."`);

    // Test 4: Query with embedding (vector search)
    console.log("\n[TEST 4] Testing vector similarity search...");
    const queryEmbedding = await createSemanticEmbedding("leather wallet", {
      fallbackToDeterministic: true,
    });

    // Use raw SQL to test vector search
    ...
    if (searchError) {
      console.log(`⚠ Vector search test: ${searchError.message}`);
      console.log(`  (This is expected if ai_corpus_rrf_search function needs the embedding fix)`);
    } else {
      console.log(`✓ Vector search completed successfully`);
      console.log(`  - Results found: ${searchData?.length || 0}`);
      if (searchData && searchData.length > 0) {
        console.log(`  - Top result: "${searchData[0].chunk?.substring(0, 50)}..."`);
      }
    }
    ...
    if (countError) {
      ...
      console.log(`✓ Total records in ai_corpus: ${totalCount}`);
      if (totalCount === 0) {
        console.log(`  ⚠ WARNING: ai_corpus is empty! Run ingestion jobs to populate it.`);
      } else if (totalCount === 1) {
        console.log(`  ℹ Only test record exists. Run ingestion jobs to populate with real data.`);
      }
    }
    ...
    const { data: samples, error: samplesError } = await supabase
      .from("ai_corpus")
      .select("id, source_type, owner_scope, marketplace, chunk")
      .order("created_at", { ascending: false })
      .limit(5);

    if (samplesError) {
      console.log(`⚠ Sample query failed: ${samplesError.message}`);
    } else if (samples && samples.length > 0) {
      console.log(`✓ Sample records (${samples.length}):`);
      samples.forEach((sample, i) => {
        console.log(`  ${i + 1}. [${sample.source_type}] ${sample.chunk.substring(0, 60)}...`);
      });
    }
    ...
    console.log("\nNext steps:");
    console.log("1. Run ingestion jobs to populate ai_corpus:");
    console.log("   - node --loader ts-node/esm jobs/ingest-metrics-to-corpus.ts");
    console.log("   - node --loader ts-node/esm jobs/ingest-predictions-to-corpus.ts");
    console.log("   - node --loader ts-node/esm jobs/ingest-risks-to-corpus.ts");
    console.log("   - node --loader ts-node/esm jobs/ingest-docs-to-corpus.ts");
    console.log("2. Verify RAG retrieval works with populated corpus");
    console.log("=".repeat(60));
  } catch (error) {
    console.error("\n" + "=".repeat(60));
