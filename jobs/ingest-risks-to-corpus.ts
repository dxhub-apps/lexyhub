...
function createRiskRuleChunk(rule: RiskRule): string {
  ...
  if (meta.auto_flag !== undefined) {
    parts.push(
      `Auto-flagging: ${meta.auto_flag ? "Enabled" : "Disabled"}`
    );
  }
  if (meta.remediation) {
    parts.push(`Remediation: ${meta.remediation}`);
  }

  return parts.join(". ");
}

function createRiskEventChunk(
  event: RiskEvent,
  ...
  if (details.trigger_reason) {
    parts.push(`Reason: ${details.trigger_reason}`);
  }
  if (details.affected_listings !== undefined) {
    parts.push(`Affected Listings: ${details.affected_listings}`);
  }
  if (details.mitigation_suggested) {
    parts.push(
      `Suggested Action: ${details.mitigation_suggested}`
    );
  }
  if (details.impact_level) {
    parts.push(`Impact Level: ${details.impact_level}`);
  }
...
async function main() {
  ...
  console.log(
    `[${runStarted}] Starting risk ingestion to ai_corpus ${runId}`
  );
  console.log(
  ...
  try {
    ...
    const { data: rules, error: rulesError } = await supabase
      .from("risk_rules")
      .select(
        "id, rule_code, description, marketplace, severity, metadata"
      );

    if (rulesError) {
      console.warn(
        `[WARN] Failed to fetch risk rules: ${rulesError.message}`
      );
    } else if (rules && rules.length > 0) {
      console.log(
        `[INFO] Processing ${rules.length} risk rules`
      );
      ...
      for (const rule of rules as RiskRule[]) {
        try {
          ...
          if (upsertError) {
            console.error(
              `[ERROR] Failed to upsert rule ${rule.rule_code}: ${upsertError.message}`
            );
            totalErrors++;
          } else {
            totalSuccess++;
          ...
          console.error(
            `[ERROR] Failed to process rule ${rule.rule_code}: ${error}`
          );
          totalErrors++;
        }
      }
      ...
      console.log(
        `[INFO] Risk rules ingestion: ${totalSuccess} success, ${totalErrors} errors`
      );
    }
    ...
    lookbackDate.setDate(
      lookbackDate.getDate() - LOOKBACK_DAYS
    );
    ...
    if (eventsError) {
      console.warn(
        `[WARN] Failed to fetch risk events: ${eventsError.message}`
      );
    } else if (events && events.length > 0) {
      console.log(
        `[INFO] Processing ${events.length} risk events`
      );
      ...
      for (const event of events as RiskEvent[]) {
        try {
          const keyword = event.keyword_id
            ? keywordsMap.get(event.keyword_id) || null
            : null;
          const rule = event.rule_id
            ? rulesMap.get(event.rule_id) || null
            : null;

          const chunk = createRiskEventChunk(
            event,
            keyword,
            rule
          );
          const embedding =
            await createSemanticEmbedding(chunk);
          ...
          if (upsertError) {
            console.error(
              `[ERROR] Failed to upsert event ${event.id}: ${upsertError.message}`
            );
            eventErrors++;
          } else {
            eventSuccess++;
            if (eventSuccess % 20 === 0) {
              console.log(
                `[INFO] Processed ${eventSuccess}/${events.length} events`
              );
            }
          }
        } catch (error) {
          console.error(
            `[ERROR] Failed to process event ${event.id}: ${error}`
          );
          eventErrors++;
        }
      }
      ...
      console.log(
        `[INFO] Risk events ingestion: ${eventSuccess} success, ${eventErrors} errors`
      );
    ...
    const duration =
      new Date(runEnded).getTime() -
      new Date(runStarted).getTime();

    console.log(
      `[${runEnded}] Risk ingestion completed`
    );
    console.log(
      `[INFO] Duration: ${(duration / 1000).toFixed(2)}s`
    );
    console.log(
      `[INFO] Total Success: ${totalSuccess}, Total Errors: ${totalErrors}`
    );
  } catch (error) {
    console.error(
      `[ERROR] Fatal error in risk ingestion: ${error}`
    );
    process.exit(1);
  }
}
