...
function createMetricChunk(
  ...
  if (dailyMetrics.length > 0) {
    ...
    const dailySummary = recentDaily.map((m) => {
      ...
      if (m.competition_score !== null) {
        metrics.push(`competition=${m.competition_score.toFixed(2)}`);
      }
  ...
  if (weeklyMetrics.length > 0) {
    ...
    parts.push(
      `Weekly Data Points: ${recentWeekly.length} weeks of historical data available`
    );
  }
...
async function main() {
  ...
  try {
    ...
    console.log(
      `[INFO] Fetching keywords updated since ${lookbackDate.toISOString()}, limit ${BATCH_SIZE}...`
    );
    ...
    // Fetch daily metrics
    console.log(
      `[INFO] Fetching daily metrics for ${keywordIds.length} keywords...`
    );
    const { data: dailyMetrics, error: dailyError } = await supabase
      .from("keyword_metrics_daily")
      .select(
        "keyword_id, collected_on, demand, supply, competition_score, trend_momentum, social_mentions, social_sentiment"
      )
    ...
    if (dailyError) {
      console.warn(
        `[WARN] Failed to fetch daily metrics: ${dailyError.message}`
      );
    } else {
      console.log(
        `[INFO] Found ${dailyMetrics?.length || 0} daily metric records`
      );
    }

    // Fetch weekly metrics
    console.log(
      `[INFO] Fetching weekly metrics for ${keywordIds.length} keywords...`
    );
    ...
    if (weeklyError) {
      console.warn(
        `[WARN] Failed to fetch weekly metrics: ${weeklyError.message}`
      );
    } else {
      console.log(
        `[INFO] Found ${weeklyMetrics?.length || 0} weekly metric records`
      );
    }
    ...
    console.log(
      `[INFO] Processing ${keywords.length} keywords with embeddings...`
    );
    for (const keyword of keywords as Keyword[]) {
      try {
        ...
        console.log(
          `[INFO] Processing keyword "${keyword.term}" (${keyword.id}): ${daily.length} daily, ${weekly.length} weekly metrics`
        );

        // Create factual chunk
        const chunk = createMetricChunk(keyword, daily, weekly);
        console.log(
          `[INFO] Created chunk for "${keyword.term}" (${chunk.length} chars)`
        );

        // Generate semantic embedding (single-argument API)
        const embedding = await createSemanticEmbedding(chunk);
        console.log(
          `[INFO] Generated embedding for "${keyword.term}" (${embedding.length} dimensions)`
        );
        ...
        if (upsertError) {
          console.error(
            `[ERROR] Failed to upsert keyword ${keyword.id} "${keyword.term}":`,
            {
              keyword_id: keyword.id,
              keyword_term: keyword.term,
          ...
          successCount++;
          console.log(
            `[INFO] ✓ Successfully inserted keyword "${keyword.term}" (${successCount}/${keywords.length})`
          );
        }
      } catch (error) {
        console.error(
          `[ERROR] Exception processing keyword ${keyword.id} "${keyword.term}":`,
          error
        );
        errorCount++;
      }
    ...
    console.log(
      `[INFO] Processing complete: ${successCount} success, ${errorCount} errors out of ${keywords.length} total`
    );
    ...
    console.error(
      `[ERROR] Fatal error in metric ingestion: ${error}`
    );
    process.exit(1);
  }
}
