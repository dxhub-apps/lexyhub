...
create index if not exists embeddings_vector_idx on public.embeddings using hnsw (embedding vector_cosine_ops) with (
    m = 16,
    ef_construction = 64
);
...
create index if not exists concept_clusters_centroid_idx on public.concept_clusters using hnsw (
    centroid_vector vector_cosine_ops
) with (
    m = 16,
    ef_construction = 64
);
