...
- Added configuration debugging in development mode
  - Logs API key (first 12 chars), host, and environment in development
- Added instance detection (US vs EU) with informational logging
...
- Provides detailed error messages explaining:
  ...
  - Revoked keys or deleted projects
- Shows current configuration (host and key format) with step-by-step solutions
- Aligned with official PostHog documentation (using ingestion endpoint format)
