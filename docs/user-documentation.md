...
- **User menu:** Provides profile management, theme switching, how-to documentation, and a logout control. Focus indicators replace hover effects so keyboard and touch users can clearly see the active option. If your profile picture hasn't been set yet, the header now falls back to a LexyHub-branded avatar and automatically recovers from broken or missing image links.
