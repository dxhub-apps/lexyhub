...
- **Search reliability guardrails:** Search now normalizes the market/source data that lands in the golden keywords table, hydrates embeddings directly from the shared cache inside the Supabase ranking RPC, aligns the tier filter with the numeric column so result sets populate immediately, and automatically falls back to an application-level ranking pass if the RPC encounters a missing column or permission error. The fallback path deterministically hydrates embeddings for the sampled keywords so placeholder records no longer block searches or create duplicate rows when someone repeats the same phrase. Source filters are also sanitized inside the RPC via the `lexy_cast_sources` helper so malformed arrays never trigger "function … does not exist" SQL errors when toggling data providers, and plan-tier filters are validated via the restored `lexy_cast_plan_tiers` helper so RPC calls never fail due to missing casts even when requests omit or corrupt the tier payload. Both helpers are now recreated automatically in every deployment, and the `lexy_rank_keywords` RPC is forcibly rebuilt to hydrate embeddings through the cache while the migration validates that every helper exists, so the search endpoint verifies its dependencies before delegating work to Supabase.
