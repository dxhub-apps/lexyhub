...
- **Search reliability guardrails:** Search now normalizes the market/source data that lands in the golden keywords table and aligns the Supabase ranking RPC with the numeric tier column, so result sets populate immediately and placeholder keywords no longer create duplicate records when a user searches for the same phrase twice.
